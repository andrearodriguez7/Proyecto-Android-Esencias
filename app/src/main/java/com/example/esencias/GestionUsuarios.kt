...
class GestionUsuarios : Fragment() {
    ...
    override fun onCreateView(
        ...
        botonInsertar.setOnClickListener {
            ...
            val tlfn = view.findViewById<EditText>(R.id.InsertarTelefonoUsuario).text.toString()
            val fotoPerfil = view.findViewById<EditText>(R.id.InsertarUrlUsuario).text.toString()

            val resultado = db.insertarUsuario2(correo, nombre, pass, tlfn, fotoPerfil)
            when (resultado) {
        ...
        botonModificar.setOnClickListener {
            ...
            val tlfn = view.findViewById<EditText>(R.id.ModificarTelefonoUsuario).text.toString()
            val fotoPerfil = view.findViewById<EditText>(R.id.ModificarUrlUsuario).text.toString()

            if (correo.isNotEmpty()) {
                val modificado = db.modificarUsuario(correo, nombre, pass, tlfn, fotoPerfil)
                if (modificado) {
